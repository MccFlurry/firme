...
def test_clean_sale_approved():
    ...
    assert verdict.expected_cost == pytest.approx(0.04 * 0.25 * 80 + 0.04 * 0.15 * 350)
...
def test_low_price_evidence_and_counterfactual():
    ...
    assert verdict.recoverable_per_minute == pytest.approx(verdict.expected_cost * 0.7 / 10)
...
def test_confirmation_signals(status, rule):
    ...
    if status in {"desconocida", "silencio"}:
...
@pytest.mark.parametrize("changes,missing", [
    ...
    ({"phone": None, "email": None, "customer_doc": None}, {"phone", "email", "customer_doc"}),
])
def test_abstention(changes, missing):
    verdict = evaluate(sale(**changes), context("desconocida"))
...
def test_noisy_or_and_prior_counted_once():
    ...
    assert verdict.outcome_costs["reclamo"] == pytest.approx(verdict.outcome_probs["reclamo"] * 80)
    assert verdict.expected_cost == pytest.approx(sum(verdict.outcome_costs.values()))
    cohort = evaluate(sale(channel="campo"), context())
    assert cohort.expected_cost == pytest.approx(0.12 * 0.25 * 80 + 0.12 * 0.15 * 350)
    assert cohort.decision == "APROBAR"  # the prior alone never sends a clean sale to review
    assert ids(cohort) == {"R27_prior_canal"}


@pytest.mark.parametrize("amount,decision", [(14.99, "APROBAR"), (15, "REVISAR"), (59.99, "REVISAR"), (60, "RETENER")])
...
def test_counterfactual_accumulates_fixes_and_does_not_mutate():
    original = sale(price=50, install_date="2026-09-15T10:00:00-05:00")
...
@pytest.mark.parametrize("hours,triggered", [(48, False), (49, True), (-1, True)])
...
def test_alert_without_install_date_and_unrelated_confirmation():
    ...
    assert verdict.alert.deadline == "2026-09-13T22:00:00-05:00"
...
def test_catalog_and_rules_have_traceable_origins():
    ...
    for rule in rules:
        ...
        assert rule["origen"] == "SUPUESTO_DEMO" and rule["fuente"]
